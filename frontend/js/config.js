...
const CONFIG_SCHEMA = [
  ...
  },
  {
    ...
    fields: [
      ...
      // ── Per-epoch temperature variation (dT) parameters ──
      { key: '_epoch_var_heading', type: 'subheader', label: 'Per-epoch temperature variation (dT)',
        tooltip: 'Apply phase-dependent brightness modulation to model temperature changes across observation epochs (based on CTTSzdi2 algorithm).' },
      { key: 'epoch_var_enabled',      label: 'Enable epoch brightness variation',  type: 'checkbox', 
        tooltip: 'Apply per-observation-epoch temperature (dT) variation via phase-dependent brightness modulation. Cooler regions have reduced brightness; hotter regions have increased brightness.' },
      { key: 'epoch_var_mode',         label: 'Variation model (var_mode)',  type: 'select', options: [{value: 1, label: '1: Gaussian'}, {value: 2, label: '2: Cosine²'}, {value: 3, label: '3: Tanh'}, {value: 4, label: '4: Exponential'}],
        tooltip: 'Temporal envelope shape: 1=Gaussian (smooth peak), 2=Cosine² (smooth bounded), 3=Tanh (prevents negative brightness), 4=Exponential (rapid evolution).' },
      { key: 'epoch_var_lifetime',     label: 'Spot lifetime (rotation cycles)', type: 'number', min: 1, step: 1, 
        tooltip: 'Typical spot/feature lifetime in stellar rotation cycles. Controls the timescale of brightness variation. Default: 100 cycles.' },
      { key: 'epoch_var_kk',           label: 'Scaling parameter (kk)',      type: 'number', step: 0.1, 
        tooltip: 'Scaling factor for phase-to-time conversion in the envelope function. Default: 10.0.' },
      { key: 'epoch_var_dphi',         label: 'Phase offset (dphi, cycles)',  type: 'number', step: 0.01, 
        tooltip: 'Rotational phase offset of the feature center (in cycles). Use to shift the timing of the brightness variation relative to phase=0.' },
      { key: 'epoch_var_dt',           label: 'dT scale', type: 'number', step: 0.1,
        tooltip: 'Amplitude scale for epoch variation. 1.0 keeps the default modulation strength; 0.0 disables the modulation.' },
    ],
  },
  {
