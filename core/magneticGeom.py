...
def _lpmn_compat(m_max, n_max, x):
     """Compute associated Legendre polynomials and their derivatives,
     reproducing the interface of the removed scipy.special.lpmn.

     Returns (p, pd) where:
       p[m, n]  = P_n^m(x)   for m in [0..m_max], n in [0..n_max]
       pd[m, n] = dP_n^m/dx
     """
     p = np.zeros((m_max + 1, n_max + 1))
     pd = np.zeros((m_max + 1, n_max + 1))

     # Compute polynomial values using scipy.special.lpmv (still available)
     for m_val in range(m_max + 1):
          n_arr = np.arange(m_val, n_max + 1, dtype=float)
          if len(n_arr) > 0:
               p[m_val, m_val:n_max + 1] = scipy.special.lpmv(m_val, n_arr, x)

     # Compute derivatives via recurrence:
     # (1 - x^2) dP_n^m/dx = -n*x*P_n^m + (n+m)*P_{n-1}^m
     ...
     if abs(denom) > 1e-14:
          for m_val in range(m_max + 1):
               for n_val in range(m_val, n_max + 1):
                    ...
                    pd[m_val, n_val] = (
                         n_val * x * p[m_val, n_val] - (n_val + m_val) * pm1
                    ) / denom
     else:
          # x = ±1: for m=0 use limiting form P_n(±1) = (±1)^n,
          # dP_n/dx|_{x=1} = n(n+1)/2
          s = 1 if x > 0 else -1
     ...
     def initMagGeom(self, clat, lon):
          ...
          for i in range(nCells):
               ...
               if ((cosClat[i] == cosClat[i-1])):
                    ...
                    pLeg_vals, pLeg_derivs = _lpmn_compat(self.nl, self.nl, cosClat[i])
                    pLegendre_all[...,i] = pLeg_vals[indTri]
                    dPdTh_all[...,i] = pLeg_derivs[indTri]
